...
class RevolveCheckpointSchedule(CheckpointSchedule):
    ...
    def __init__(self, max_n, snap_in_ram, snap_on_disk, f_cost=1.0, b_cost=1.0, 
                 wvec=(0, 0.5), rvec=(0, 0.5)):
    ...
    def iter(self):
        """An iterator object used to execute the forward and adjoint compuations 
        by following the revolve schedule.
    ...
    def is_exhausted(self):
        ...
        -------
        bool
            If `True` then the `EndReverse` action should be activated.
        """
    ...
    def uses_disk_storage(self):
        ...
        -------
        bool
            Whether the "DISK" storage is being used to store the checkpoint data.
        """
...
def _convert_action(action):
    ...
    Notes
    -----
    This function provide the storage level where the forward checkpoint 
    data will be stored according the action provides by the 
    H-revolvve sequence. The storage level are either "RAM" or "DISK".

    Returns
    -------
    str, tuple(int, int , str)
        Return the action name, step 'n_0' and 'n_1' associated to the action, and the storage level.
