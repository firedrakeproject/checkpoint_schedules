...
class RevolveCheckpointSchedule(CheckpointSchedule):
    ...
    def iter(self):
        """An iterator object used to execute the forward and adjoint computations. 
        ...
        Notes
        -----
        The actions and the data `n0`, `n1` and `sorage` follow the H-Revolve schedule.
...
def _convert_action(action):
    ...
    Notes
    -----
    This function provides the storage level where the forward checkpoint 
    data will be stored according the action provides by the 
    H-revolve sequence. The storage levels are either `RAM` or `disk`.

    Returns
    -------
    str, tuple(int, int , str)
        Return the action name, steps `n_0` and `n_1`.
